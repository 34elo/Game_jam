...
def play(f):
    class Game():
        def __init__(self):
            ...
            pygame.display.set_caption("Hangover4")
            ...
            if  f == 1:
                ...
                self.map = load_pygame(join('data', 'maps', 'asasin.tmx'))
            #
        ...
        def load_images(self):
            ...
            if f == 3:
                    folders = list(walk(join('images', 'enemies1')))[0][1]
                    self.enemy_frames = {}
                    for folder in folders:
                        for folder_path, _, file_names in walk(join('images', 'enemies1', folder)):
                            self.enemy_frames[folder] = []
        ...
        def setup(self):
            ...
            for obj in self.map.get_layer_by_name("object 2"):
                if obj.name == "Player":
                    self.player = Player((obj.x, obj.y), self.all_sprites, self.collision_sprites, f)
                    self.gun = Gun(self.player, self.all_sprites, f)
        ...
        def bullet_collision(self):
            ...
            if self.kills == 0:
                if f == 1:
                    Cutscenes.first_cut(2)
                if f == 3:
                    Cutscenes.first_cut(4)
        ...
        def health(self, dt):
            ...
            for _ in range(self.x):
                for i in range(4):
                    im = pygame.image.load(join('images', 'Hp', '4.png'))
                    ...
                    if event.type == pygame.KEYDOWN and event.key == pygame.K_q:
                        ...
                        if is_point_in_rectangle(a[0], a[1], a[2], a[3], b[0], b[1]):
                            ...
                            if f == 1:
                                ...
                                self.setup()
                            if f == 3:
                                self.map = load_pygame(join('data', 'maps', 'dungeon1.tmx'))
                                self.display_surface.fill('black')
                                pygame.display.flip()
                                self.setup()
